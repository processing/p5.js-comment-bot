...
router.post('/webhooks', async (c, next) => {
	...
	if(
		payload.workflow_run.name === 'Publish approved pull requests and latest commit to pkg.pr.new' &&
		...
		const tokenRes = await request(`https://api.github.com/app/installations/${installationID}/access_tokens`, {
			method: 'POST',
			headers: {
		...
		const commentsRes = await request(`https://api.github.com/repos/processing/p5.js/issues/${data.workflow.pull_request.number}/comments`, {
			headers: {
				Authorization: `Bearer ${token}`
			}
		});
		...
		if(previousComment){
			...
			const newComment = updateComment(previousComment.body, data.packages, payload.workflow_run.head_sha.substring(0, 7));

			await request(`https://api.github.com/repos/processing/p5.js/issues/comments/${commentID}`, {
				method: 'PATCH',
				headers: {
					Authorization: `Bearer ${token}`
				},
...
_This is an automated message._`;
			...
			await request(`https://api.github.com/repos/processing/p5.js/issues/${data.workflow.pull_request.number}/comments`, {
				method: 'POST',
				headers: {
					Authorization: `Bearer ${token}`
				},
